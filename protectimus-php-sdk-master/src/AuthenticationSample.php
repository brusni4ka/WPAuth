...
define("USERNAME", "katerynahovor@gmail.com");
define("API_KEY", "YAiZbqBDDLdrTb0otXnM5XE72ciFAFyp");
...
/*
	...
	$response = $api->addSoftwareToken(null, null, "MAIL",
					"govor94@bk.ru", "Mail token", null, null, 6, null, null, null);
	echo print_r($response, true) . "<br/>";
	$tokenId = $response->response->id;
	echo "ID OF THE CREATED TOKEN =>>> " . $tokenId . "<br/>";

	echo "<br/>ASSIGNING TOKEN WITH ID = " . 4842
	. " TO RESOURCE WITH ID = " . $resourceId . "<br/>";
	// assigns token to resource
	$response = $api->assignTokenToResource(100, null, 4842);
	echo print_r($response, true) . "<br/>";
	...
	$response = $api->prepareAuthentication(100, null, null, 'medved');
	echo print_r($response, true) . "<br/>";
...
/*	// one-time password
	...
	*/
	...
	// for
	// MAIL-token or get challenge string for PROTECTIMUS_OCRA-token.
	$response = $api->prepareAuthentication($resourceId, null, $userId, null);
	echo print_r($response, true) . "<br/>";
/*
	echo "<br/>AUTHENTICATION WITH ONE-TIME PASSWORD<br/>";
